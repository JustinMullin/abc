...
import com.jmullin.drifter.entity.{Layer, Layer2D, Layer3D}
...
abstract class DrifterScreen(game: DrifterGame, background: Color=Color.BLACK) extends DrifterInput with Screen {
  ...
  def render(delta: Float) {
    ...
    if(game.showFps) {
      ...
      batch.begin()
      Assets.petMe12.setColor(Color.YELLOW)
      Assets.petMe12.draw(batch, "FPS: " + Gdx.graphics.getFramesPerSecond, 3, gameH-3)
      batch.end()
    }
  }

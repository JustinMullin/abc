...
object Settings {
  ...
  lazy val desktop = core ++ Seq(
    libraryDependencies ++= Seq(
      ...
      "com.badlogicgames.gdx" % "gdx-tools" % libgdxVersion.value,
  ...
  lazy val ios = core ++ Tasks.natives ++ Seq(
    ...
    <config>
      ...
      <libs>
        <lib>lib/libgdx.a</lib>
        <lib>lib/libObjectAL.a</lib>
      </libs>
      <resources>
        <resource>
    ...
    libraryDependencies ++= Seq(
      ...
      "com.badlogicgames.gdx" % "gdx-box2d-platform" % libgdxVersion.value % "natives" classifier "natives-ios"
    )/*,
    nativeExtractions <<= baseDirectory { base => Seq(
